...
def evaluate_one_image(image_array):
    with tf.Graph().as_default():
        ...
        image = tf.reshape(image, [1, 74,74, 3])

        logit = model.inference(image, BATCH_SIZE, N_CLASSES)

        logit = tf.nn.softmax(logit)

        x = tf.placeholder(tf.float32, shape=[74, 74, 3])

        # you need to change the directories to yours.
        logs_train_dir = '/content/gdrive/My Drive/twokidneyca/savev2'
...
if __name__ == '__main__':
    img = Image.open('/content/gdrive/My Drive/twokidneyca/inputdata/12.png')
    plt.imshow(img)
    plt.show()
    imag = img.resize([74, 74])
    image = np.array(imag)
